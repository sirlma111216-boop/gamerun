...
window.addEventListener('message',e=>{if(e.source!==parent||!allowed||e.origin!==allowed)return;parentOrigin=e.origin;const m=e.data;
 if(m?.type==='lumi:mount'){game?.destroy();const c=(m.config||{}) as LumiConfig;const emit=(type:string,value:unknown)=>parent.postMessage({type,value},parentOrigin!);game=mountLumiRun(document.getElementById('app')!,{...c,onReady:s=>emit('lumi:ready',s),onStart:s=>emit('lumi:start',s),onEnd:r=>emit('lumi:end',r),onResult:r=>emit('lumi:result',r),onLobby:s=>emit('lumi:lobby',s)});}
